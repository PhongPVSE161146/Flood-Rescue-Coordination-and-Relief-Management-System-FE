...
/>
  ...
  onChange={async ({ file, fileList }) => {
    ...
    // nếu file đã check rồi thì bỏ qua
    if (file.status === "done") return;
  
    try {
  
      AuthNotify.info("Đang kiểm tra ảnh bằng AI...");
  
      const res = await verifyFloodImage(file.originFileObj || file);
  
      if (!res?.isValid) {
  
        AuthNotify.error(
          "Ảnh không hợp lệ",
          res?.message || "Ảnh không phải bão/lũ"
        );
  
        // ❌ remove ảnh sai khỏi list
      ...
      AuthNotify.success("Ảnh hợp lệ", res.message);
